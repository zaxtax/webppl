...
module.exports = function(top){
  ...
  PMCMC.prototype.resetParticles = function(){
    ...
    for (var i=0; i<this.numParticles; i++) {
      var particle = {
        continuations: [function(s){that.wpplFn(s, top.exit, that.address);}],
      ...
      if (this.sweep > 0) {
        this.particles.concat(this.retainedParticle).forEach(
          function(particle){
            ...
            if (top.coroutine.returnHist[k] === undefined){
              top.coroutine.returnHist[k] = { prob:0, val:particle.value };
            }
            top.coroutine.returnHist[k].prob += 1;
          });
      }
